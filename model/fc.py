...
class FC(nn.Module):
    def __init__(self, dims, num_class=55):
        ...
        self.fc1 = nn.Sequential(*[nn.Linear(dims[i], dims[i+1]) for i in range(len(dims)-1)]) 
...
class NetVLAD(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(batch_size):
            ...
            residual = x_point.expand(self.num_clusters, -1, -1, -1).permute(1, 0, 2, 3) - \
            self.centroids.expand(x_point.size(-1), -1, -1).permute(1, 2, 0).unsqueeze(0)
            #print(residual)
...
class NetVLADLoupe(nn.Module):
    def __init__(self, feature_size=256, max_samples=4000, cluster_size=64, output_dim=1024,
