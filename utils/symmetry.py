...
def visual_symmetry_poins(feat, raw_pc, i, T):
    ...
    kmeans = KMeans(n_clusters=2, random_state=0).fit(nns)
...
def split_corr(pcsA, pcsB, featsA, featsB, knn, subsample_size=1000):
    ...
    xyzA_corrs = torch.cat(xyzA_corrs, 0)
    xyzB_corrs = torch.cat(xyzB_corrs, 0)
...
def symmetric_cut4(feat, raw_pc, K, max_sample=100):
    ...
    for i in range(max_sample):
        ...
        local_dist = (feat[index:index+1, :] - feat).norm(2, 1).detach().cpu().numpy()
        ...
        kmeans = KMeans(n_clusters=K, random_state=0).fit(nns)
        ...
        if dist.min() > 0.15 and max(error) < 0.15 and curr_var > np.sqrt(np.var(ratios)):
    ...
    if K == 2:
        ...
        return [labels==0, labels==rank_centers[0]+1, labels==rank_centers[2]+1, labels==rank_centers[1]+1]
    else:
...
def sym_pose(baseF, xyz0, posF, xyz1, pos_sym, k_nn=5, max_corr=0.20):
    ...
    chamf_dist_ransac = chamfer_gpu_1direction(apply_transform(xyz0, T_est_ransac).cuda(), xyz1.cuda())
    ...
    try:
        ...
        print("symmetry failed use ransac")
        return T_est_best, chamf_dist_best, T_est_ransac, chamf_dist_ransac
