...
def retrieval_dist(dists, threshold, table):
    ...
    for d, p, g, t in zip(dists, rank_pd, rank_gt, table):
        ...
        percision.append(100. * np.sum(positive)/accept_range)
        if np.sum(positive) == 0:
            score = 0
        else:
            score = 100. * average_precision_score(positive, 2 - d[p])
        scores.append(score)
...
def scan2cad_retrieval_eval_dist(dists, table, best_match, pos_n):
    ...
    for g, p in zip(gt_rank, pred_rank):
        ...
        percision.append(100. * np.sum(positive)/pos_n)
    ...
    stat = {"percision": sum(percision)/len(percision), "top1_error": sum(top1_error)/len(top1_error), 
            "top1_predict": top1_predict, "gt": gt}
