...
class Scan2cadInfo:
    def __init__(self, cad_root, scan_root, catid, annotation_dir):
        ...
        self.ScanObjPathes, self.BestMatchesDict, self.CadPosesDict, self.ScanPosesDict = self.read_scans()
        self.ScanObjPathes.sort()
                
        path_train = "./config/scannet_train.txt"
        path_test = "./config/scannet_val.txt"
        ...
        self.train_files, self.TrainBestMatches, self.TrainCadPoses, self.TrainScanPoses = self.keys_to_label(self.train_keys, self.scene_dict)
        self.val_files, self.ValBestMatches, self.ValCadPoses, self.ValScanPoses = self.keys_to_label(self.val_keys, self.scene_dict)
        self.test_files, self.TestBestMatches, self.TestCadPoses, self.TestScanPoses = self.keys_to_label(self.test_keys, self.scene_dict)
        
    def get_split(self, split):
        if split == "train":
            return self.train_files, self.TrainBestMatches, self.TrainCadPoses, self.TrainScanPoses
        elif split == "val":
            return self.val_files, self.ValBestMatches, self.ValCadPoses, self.ValScanPoses
        elif split == "test":
            return self.test_files, self.TestBestMatches, self.TestCadPoses, self.TestScanPoses
        else:
    ...
    def print_stats(self):
        ...
        print("Scans:")
        print("\tnumber of scenes train: {}, val: {}, test: {}".format(len(self.train_keys), len(self.val_keys), len(self.test_keys)))
        print("\tnumber of objects train: {}, val: {}, test: {}".format(len(self.train_files), len(self.val_files), len(self.test_files)))
    ...
    def read_scans(self):
        ...
        with open("./config/scannet_omit.txt", "r") as f:
            lines = f.readlines()
            omittion = [line.strip('\n') for line in lines]
        ...
        for file in files:
            ...
            if not file in omittion and CatId == self.catid:
                pcs.append(file)
    ...
    def read_scan2pose(self):
        ...
        for scan in self.annotation:
            scan2pose[scan["id_scan"]] = {"aligned_models": scan["aligned_models"], "trs": scan["trs"]}
