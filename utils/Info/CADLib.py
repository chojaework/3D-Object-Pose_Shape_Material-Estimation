...
class CatCADLib():
    ...
    def __init__(self, root, catid, table_path, preload=True):
        ...
        CadReader = CategoryLibReader(self.root, self.catid, ["train", "test", "val"], 10000, normal=False)
        CadReaderLoader = torch.utils.data.DataLoader(CadReader, batch_size=1, shuffle=False, num_workers=8)
...
class CustomizeCADLib(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        base = {"coord": rot_base_coords_grid, "origin":rot_base_coords, "feat": base_feat, "T": identity, "idx": idx}
