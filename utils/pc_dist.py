...
import threading
import argparse
from utils.preprocess import read_file, read_split, print_stat, load_raw_pc, load_norm_pc
...
def compute_dist(pcs):
    ...
    for base_idx in tqdm(range(len(pcs))):
        ...
        if total > 8:
            for i in range(8):
                domain = [base_idx+1+i*total//8, base_idx+1+(i+1)*total//8]
                t = threading.Thread(target=compute_chamfer, args=(base_idx, pcs[base_idx], pcs, domain, table))
                t.start()
                threds.append(t)
        else:
            t = threading.Thread(target=compute_chamfer, args=(base_idx, pcs[base_idx], pcs, [base_idx+1, len(pcs)], table))
            t.start()
            threds.append(t)
...
def shapenet_cat(split, catid):
    ...
    for _, objid in enumerate(files):
        pcs.append(torch.Tensor(load_norm_pc(os.path.join(root, catid, split, objid), 2000), device=torch.device('cuda')))
