...
if __name__ == "__main__":
    # path
    scan_root = "/scannet/scans_test"
    cad_root = "/zty-vol/data/ShapeNetCore.v2.PC15k"
    scan2cad = load_json("/scannet/scan2cad_download_link/full_annotations.json")
    target_dir = "/scannet/crop_scan2cad_filter/test"
    ...
    for idx in range(len(scan2cad)):
                
        scannetid = scan2cad[idx]["id_scan"]

        print("reading scan: {}".format(scannetid))

        if not os.path.exists(os.path.join(scan_root, scannetid, "{}_vh_clean.aggregation.json".format(scannetid))):
            continue
        ...
        for i, aligned_model in enumerate(scan2cad[idx]["aligned_models"]):
            ...
            if os.path.exists(os.path.join(target_dir, "{}.{}.{}.{}.npy".format(scannetid, i, aligned_model["catid_cad"], aligned_model["id_cad"]))):
                continue
                
            cad_path = id2path[aligned_model["id_cad"]]
            #cad_pc = load_raw_pc(cad_root, cad_path, 10000)
            ...
            if dist[nearest] >= 0.3:
                invalid_count += 1
                print("{} Large error. Abort.".format(invalid_count))
                continue
            
            scan2cad_count += 1
            print("{} valid".format(scan2cad_count))
            ...
            np.save(os.path.join(target_dir, "{}.{}.{}.{}.npy".format(scannetid, i, aligned_model["catid_cad"], aligned_model["id_cad"])), np.asarray(pcd.points))
