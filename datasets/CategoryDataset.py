...
class CategoryDataset(ChairDataset):
    ...
    """
    def __init__(self, root, split, catid, dist_mat_root, pos_ratio, neg_ratio, voxel_size):
        self.root = root
        self.split = split
        self.catid = catid
        ...
        dist_mat_ref = np.load(os.path.join(self.dist_mat_root, "{}_{}.npy".format(catid, split)))
        ...
        # Load symmetry label
        if os.path.exists("./config/{}_{}_rot_sym_label.txt".format(self.catid, self.split)):

            with open("./config/{}_{}_rot_sym_label.txt".format(self.catid, self.split), "r") as f:
        ...
        self.dist_mat, self.pcs, self.sym_label = self.filter_data(dist_mat_ref, pcs_ref, sym_ref)
    ...
    def quant(self, rot_coords, coords):
        ...
        """
        floor = np.floor(rot_coords / self.voxel_size)
        np_cont = np.ascontiguousarray(floor)
        _, unique_idx = ME.utils.sparse_quantize(np_cont, return_index=True) 
    ...
    def __getitem__(self, index):
        ...
        while(not isinstance(pos_inst_pos_pairs, np.ndarray) or not isinstance(pos_inst_neg_pairs, np.ndarray) or not isinstance(neg_inst_neg_pairs, np.ndarray)):
            ...
            pos_inst_pos_pairs, pos_inst_neg_pairs, neg_inst_neg_pairs = self.generate_local_pair(base_coords, pos_coords, neg_coords)
        ...
        base = {"coord": rot_base_coords_grid, "origin":rot_base_coords, "feat": base_feat, "T": base_trans, "idx":idx, "sym":base_sym}
        pos = {"coord": rot_pos_coords_grid, "origin":rot_pos_coords, "feat": pos_feat, "T": pos_trans, "idx":positive_idx, "sym":pos_sym}    
        neg = {"coord": rot_neg_coords_grid, "origin":rot_neg_coords, "feat": neg_feat, "T": neg_trans, "idx":negative_idx, "sym":neg_sym}
        
        return base, pos, neg, pos_inst_pos_pairs, pos_inst_neg_pairs, neg_inst_neg_pairs
