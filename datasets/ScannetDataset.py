...
class ScannetDataset(CategoryDataset):
    ...
    """
    def __init__(self, scan_root, cad_root, CADLib, Scan2CadInfo, split, catid, pos_ratio, neg_ratio, voxel_size, preload=True):
        ...
        self.files, self.BestMatches, self.CadPoses, self.ScanPoses = Scan2CadInfo.get_split(split)
        ...
        if os.path.exists("./config/{}_{}_rot_sym_label.txt".format(self.catid, "scan2cad")):

            with open("./config/{}_{}_rot_sym_label.txt".format(self.catid, "scan2cad"), "r") as f:
    ...
    def __getitem__(self, idx):
        ...
        while pos_inst_pos_pairs is None:
            ...
            T_base = to_T(self.ScanPoses[idx]["translation"], self.ScanPoses[idx]["rotation"],
                                    self.ScanPoses[idx]["scale"])
            
            T_pos = to_T(self.CadPoses[idx]["translation"], self.CadPoses[idx]["rotation"], 
        ...
        base = {"coord": rot_base_coords_grid, "origin":rot_base_coords, "feat": base_feat, "T": base_trans, "idx":idx, "sym":1}
        pos = {"coord": rot_pos_coords_grid, "origin":rot_pos_coords, "feat": pos_feat, "T": pos_trans, "idx":self.id2idx[self.BestMatches[idx]], "sym":pos_sym}    
        neg = {"coord": rot_neg_coords_grid, "origin":rot_neg_coords, "feat": neg_feat, "T": neg_trans, "idx":negative_idx, "sym":1}
