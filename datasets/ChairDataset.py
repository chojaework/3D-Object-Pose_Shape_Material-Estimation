...
class ChairDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, root, split, pos_ratio, neg_ratio, voxel_size):
        ...
        self.root = root
        self.file_list, self.lable2data, self.data2label = read_label("./config/label_{}".format(split))
    ...
    def collate_pair_fn(self, list_data):
        ...
        base_dict, pos_dict, neg_dict, pos_inst_pos_pairs, pos_inst_neg_pairs, neg_inst_neg_pairs = list(zip(*list_data))
        ...
        base_T = [];pos_T = [];neg_T = []
        base_origin = [];pos_origin = [];neg_origin = []
        base_idx = [];pos_idx = [];neg_idx = []
        base_sym = [];pos_sym = [];neg_sym = []
        ...
        for idx in range(len(base_dict)):
            ...
            PiP.append(torch.from_numpy(np.array(pos_inst_pos_pairs[idx]) + pos_curr_idx))
            PiN.append(torch.from_numpy(np.array(pos_inst_neg_pairs[idx]) + pos_curr_idx))
            NiN.append(torch.from_numpy(np.array(neg_inst_neg_pairs[idx]) + neg_curr_idx))
