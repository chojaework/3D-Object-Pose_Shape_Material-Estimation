...
class HardCategoryDataset(ChairDataset):
    def __init__(self, root, split, catid, table_root, pos_ratio, neg_ratio, voxel_size):
        self.root = root
        self.split = split
        self.catid = catid
        ...
        table_ref = np.load(os.path.join(self.table_root, "{}_{}.npy".format(catid, split)))
    ...
    def __getitem__(self, index):
        ...
        rot_pos_coords = [];pos_trans = []
        rot_neg_coords = [];neg_trans = []
        ...
        for i in range(len(pos_coords)):
            rot_coords, rot_coords_grid, coords = self.quant(rot_pos_coords[i], pos_coords[i])
            rot_pos_coords[i] = rot_coords
            pos_coords[i] = coords
            rot_pos_coords_grid.append(rot_coords_grid)
            
        for i in range(len(neg_coords)):
            rot_coords, rot_coords_grid, coords = self.quant(rot_neg_coords[i], neg_coords[i])
        ...
        pos_feat = [np.ones([len(rot_pos_coords[i]), 1]) for i in range(len(rot_pos_coords))] 
        neg_feat = [np.ones([len(rot_neg_coords[i]), 1]) for i in range(len(rot_neg_coords))] 

        # tag: data output
        base = {"coord": rot_base_coords_grid, "origin":rot_base_coords, "feat": base_feat, "T": base_trans, "idx":idx}
        pos = [{"coord": rot_pos_coords_grid[i], "origin":rot_pos_coords[i], 
                "feat": pos_feat[i], "T": pos_trans[i], "idx":positive_idx[i]}
               for i in range(len(rot_pos_coords))]
        neg = [{"coord": rot_neg_coords_grid[i], "origin":rot_neg_coords[i], 
                "feat": neg_feat[i], "T": neg_trans[i], "idx":negative_idx[i]}
               for i in range(len(rot_neg_coords))]
        
        return base, pos, neg, #pos_inst_pos_pairs, pos_inst_neg_pairs, neg_inst_neg_pairs, label
    ...
    def collate_pair_fn(self, list_data):
        ...
        base_coords = [];pos_coords = [];neg_coords = []
        base_feat = [];pos_feat = [];neg_feat = []
        base_idx = [];pos_idx = [];neg_idx = []
...
if __name__ == "__main__":
    ...
    train_loader = torch.utils.data.DataLoader(H, batch_size=4, shuffle=False, 
                                                num_workers=4, collate_fn=H.collate_pair_fn)
